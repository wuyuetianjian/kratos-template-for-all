...
func (_q *PermissionQuery) QueryModule() *ModuleQuery {
...
func (_q *PermissionQuery) WithModule(opts ...func(*ModuleQuery)) *PermissionQuery {
	query := (&ModuleClient{config: _q.config}).Query()
...
//		GroupBy(permission.FieldAction).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (_q *PermissionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Permission, error) {
	...
	if query := _q.withModule; query != nil {
		if err := _q.loadModule(ctx, query, nodes, nil,
			func(n *Permission, e *Module) { n.Edges.Module = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *PermissionQuery) loadModule(ctx context.Context, query *ModuleQuery, nodes []*Permission, init func(*Permission), assign func(*Permission, *Module)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Permission)
	for i := range nodes {
		if nodes[i].module_permissions == nil {
			continue
		}
		fk := *nodes[i].module_permissions
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "module_permissions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
